...
—— 一个文件（可以带行号锚点）、一个黄金用例 id、或采纳该想法的那个提交。
`pnpm prior-art check` 会把每一条引用对着检出目录解析，而 CI 会跑它，所以重命名一个
文件会让构建失败，而不是把一条已经不再描述任何东西的主张留在一个页面上。页面在加载
时也会重新解析一次，并把它经已移动的引用标成失效。
...
- **辅助录入参考资料。** 今天往 Prior art 标签页添加项目是一个表单。抓取仓库、起草
  主张还没有做，而且真做的时候采纳与否仍然要人决定：评估是判断，只有证据必须是可机械
  校验的那部分。
