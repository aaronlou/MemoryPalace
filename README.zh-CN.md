...
> 状态：v0.1，端到端可用。263 个测试全绿。整套系统（包括评估套件）都能在 mock provider 上离线运行，因此文档里的入口路径不需要 API key、不需要联网。CI 每次 push 跑 lint、构建、测试套件，以及三遍 walkthrough，其中一遍来自全新克隆。
...
| `memory_feedback` | 上报一次召回答错或不完整 —— 要说清**漏了什么** |
...
上面每个数字，量的都是有人手写下来的问题。这就是瓶颈：黄金集的增长速度等于有人愿意打字的速度，而这个速度，也就是这个系统能被告诉\"你错了\"的速度。

所以现在可以在失败还显而易见的那一刻把它记下来。Web 界面的召回测试给出三个评价按钮，`memory_feedback` 让 agent 在答错之后也能上报同样的事。每一条都会保存**原样的提问**和**当时返回的全部内容** —— 因为"他不喜欢记忆 X"这句话，等那一版被取代之后就读不懂了。

两个命令合上这个环。`pnpm feedback review` 把积攒下来的评价打印成候选评测用例 —— 只打印，不写入：什么该进评测集是一个判断，脚本一旦能改评测集，就会同时悄悄改变此后每一次对比的含义。采纳之后用 `pnpm feedback promote <fb> <case>` 记下对应关系。没有填"缺了什么"的 `missed` 会被拒绝：它能被计数，却永远无法被处理。

这些都不会改变你的召回。召回路径不读这些行 —— 一旦读了，标注就成了系统的一部分，而不是对系统的测量。
...
| `pnpm feedback review` | 把积累的评价打印成候选评测用例 |
| `pnpm feedback promote <fb> <case>` | 记录某条评价已经变成那条黄金用例 |
