...
### 参考资料，以及为什么它是一个页面而不是一个 wiki

本项目读过的那些项目，以及从中取用了什么 —— 包括刻意没有取用的部分。它是 Web UI 的
**Prior art** 标签页，也是 `scripts/prior-art-seed.ts` 里的版本化内容。

**添加一个项目只需要一个链接。** 系统不会要你去判断某个项目值不值得借鉴 —— 那正是被
自动化的那部分。粘贴一个 GitHub 链接，后台会去读这个仓库（描述、topics 与 README，
并记录读的是哪个版本），然后让模型说明它主张什么、与本项目在哪里重叠、以及我们会刻意
**不**取用它什么。返回的是一份**由你复核的草稿**：每个字段都可以改，在你采纳之前什么
都不会进入清单。忽略的代价是零 —— 因为那条目从一开始就只存了一个链接。

让它值得一读的规则是：标为 `adopted` 或 `partial` 的条目必须指向本仓库里的某样东西
—— 一个文件（可以带行号锚点）、一个黄金用例 id、或采纳该想法的那个提交。模型会拿到
本项目的 ADR、评估用例与源码路径索引，并被要求只从中引用；它给出的每一条引用随后都会
对着检出目录解析，解析不了的那些会被**丢弃并展示给你**。这就是「引用」与「一个看起来
像文件名的字符串」的区别。`pnpm prior-art check` 对种子内容做同样的检查，CI 会跑它，
所以重命名一个文件会让构建失败，而不是把一条已经不再描述任何东西的主张留在一个页面上。

`watched` 条目需要一条终止条件，因为「我们在关注它」而没有任何退出条件，正是一份
这样的清单烂掉的方式。`rejected` 条目不需要证据：理由*就是*证据，而记录「否决」的
条目通常是更有用的那一半。

读取仓库用的是 GitHub 公共 API —— 不带 token 每小时 60 次，带 token 5000 次；如果你
会经常用到，请在 `.env` 里设置 `GITHUB_TOKEN`。

它是一张独立的表，而不是记忆库的一部分，这件事比听起来重要。一条记忆回答的是「这个
Agent 应该知道用户的什么」；这里的一条记录回答的是「算法为什么长这样」。把两者混在
一起，会让一个关于用户偏好的查询浮出一个我们只是读过的系统。
...
| `pnpm prior-art check` / `seed` / `list` | 算法背后的参考资料清单 —— `check` 会把每条主张对着本检出目录解析 |
...
- **参考资料评估的工作队列。** 评估跑在 API 进程里，一次一个。评估中途重启会让它失败
  并提供重试，而不是接着跑；起第二个实例则需要把这个队列移出进程。
